...
import org.luckymod.LuckyModEvents;
...
public class Minecraft implements IThreadListener, ISnooperInfo {
    ...
    public HashMap<Integer, Integer> keyboardCPS = new HashMap<Integer, Integer>();
    public int cpsLeft = 0;
    public int cpsRight = 0;
    ...
    int clicksPerSecLeftCounter = 0;
    int clicksPerSecRightCounter = 0;
    HashMap<Integer, Integer> keysCounter = new HashMap<>();
    ...
    private void startGame() throws LWJGLException, IOException {
        ...
        Display.setTitle("LuckyMod " + Constants.VERSION);
    ...
    private void createDisplay() throws LWJGLException {
        ...
        Display.setTitle("Loading LuckyMod " + Constants.VERSION + "...");
    ...
    public void draw(int posX, int posY, int texU, int texV, int width, int height, int red, int green, int blue, int alpha) {
        ...
        vertexbuffer.pos(posX, posY + height, 0.0D).tex((float) texU * 0.00390625F, (float) (texV + height) * 0.00390625F).color(red, green, blue, alpha).endVertex();
        vertexbuffer.pos(posX + width, posY + height, 0.0D).tex((float) (texU + width) * 0.00390625F, (float) (texV + height) * 0.00390625F).color(red, green, blue, alpha).endVertex();
        vertexbuffer.pos(posX + width, posY, 0.0D).tex((float) (texU + width) * 0.00390625F, (float) texV * 0.00390625F).color(red, green, blue, alpha).endVertex();
        vertexbuffer.pos(posX, posY, 0.0D).tex((float) texU * 0.00390625F, (float) texV * 0.00390625F).color(red, green, blue, alpha).endVertex();
    ...
    private void runGameLoop() throws IOException, AWTException {
        ...
        if (newTime - oldTime >= 1000.0) {
            oldTime = System.currentTimeMillis();
            cpsLeft = clicksPerSecLeftCounter;
            cpsRight = clicksPerSecRightCounter;
            clicksPerSecLeftCounter = 0;
            clicksPerSecRightCounter = 0;
            for (int keyCode = 0; keyCode <= Keyboard.getKeyCount(); keyCode++) {
                keyboardCPS.put(keyCode, keysCounter.get(keyCode));
                keysCounter.put(keyCode, 0);
            }
        }
        ...
        if (theWorld != null) {
            ...
            //this.drawGradientRect(2, 4, 5, 5, -1072689136, -804253680);


            String[] info = new String[]{
                    "§lFPS: §r§f " + debugFPS,
                    gameSettings.getLuckyModGson().get("toggleSprint").getAsBoolean() ? "§l[ToggleSprint] Sprint: §r§f" + (thePlayer.sprint ? "Ativado" : "Desativado") : ""
            };
            int[] infoColor = new int[]{
                    0xff9900,
                    0xff9900
            };
            for (int inde = 0; inde < info.length; inde++) {
                fontRendererObj.drawStringWithShadow(info[inde], 0, fontRendererObj.FONT_HEIGHT * inde, infoColor[inde]);
    ...
    private void displayDebugInfo(long elapsedTicksTime) {
        if (this.mcProfiler.profilingEnabled) {
            ...
            vertexbuffer.pos((float) j - 176.0F, (float) k - 96.0F - 16.0F, 0.0D).color(200, 0, 0, 0).endVertex();
            vertexbuffer.pos((float) j - 176.0F, k + 320, 0.0D).color(200, 0, 0, 0).endVertex();
            vertexbuffer.pos((float) j + 176.0F, k + 320, 0.0D).color(200, 0, 0, 0).endVertex();
            vertexbuffer.pos((float) j + 176.0F, (float) k - 96.0F - 16.0F, 0.0D).color(200, 0, 0, 0).endVertex();
            tessellator.draw();
            ...
            for (int l = 0; l < list.size(); ++l) {
                ...
                for (int j2 = i1; j2 >= 0; --j2) {
                    ...
                    vertexbuffer.pos((float) j + f1, (float) k - f2, 0.0D).color(k1, l1, i2, 255).endVertex();
                }
                ...
                for (int i3 = i1; i3 >= 0; --i3) {
                    ...
                    vertexbuffer.pos((float) j + f4, (float) k - f5, 0.0D).color(k1 >> 1, l1 >> 1, i2 >> 1, 255).endVertex();
                    vertexbuffer.pos((float) j + f4, (float) k - f5 + 10.0F, 0.0D).color(k1 >> 1, l1 >> 1, i2 >> 1, 255).endVertex();
                }
                                    ...
                                    if (enumactionresult == EnumActionResult.SUCCESS) {
                                        ...
                                        if (itemstack != null) {
                                            if (itemstack.stackSize == 0) {
                                                this.thePlayer.setHeldItem(enumhand, null);
    ...
    private boolean processKeyF3(int p_184122_1_) {
        if (p_184122_1_ == 30) {
            ...
            this.debugChatMessage("Hitboxes: {0}", flag1 ? "shown" : "hidden");
            return true;
            ...
            this.debugChatMessage("RenderDistance: {0}", Integer.valueOf(this.gameSettings.renderDistanceChunks));
            return true;
        } else if (p_184122_1_ == 34) {
            boolean flag = this.debugRenderer.func_190075_b();
            this.debugChatMessage("Chunk borders: {0}", flag ? "shown" : "hidden");
            return true;
    ...
    private void processKeyBinds() {
        ...
        for (; this.gameSettings.keyBindTogglePerspective.isPressed(); this.renderGlobal.setDisplayListEntitiesDirty()) {
            ...
            if (this.gameSettings.thirdPersonView > 2 && !this.entityRenderer.freeCamIsEnabled) {
        ...
        while (this.gameSettings.keyBindToggleFreeCam.isPressed()) {
            this.entityRenderer.freeCamIsEnabled = !this.entityRenderer.freeCamIsEnabled;
            if (this.entityRenderer.freeCamIsEnabled) {
                this.gameSettings.thirdPersonView = 3;
            } else
                this.gameSettings.thirdPersonView = 0;
        }
        ...
        while (gameSettings.keyBindToggleSneak.isPressed()) {
            gameSettings.setOptionValueLuck("toggleSneak", !gameSettings.getLuckyOptionB("toggleSneak"));
            thePlayer.addChatMessage(new TextComponentString("ToggleSneak " + (gameSettings.getLuckyOptionB("toggleSneak") ? "Ativado" : "Desativado")));

        }
        while (gameSettings.keyBindToggleSprint.isPressed()) {
            gameSettings.setOptionValueLuck("toggleSprint", !gameSettings.getLuckyOptionB("toggleSprint"));
            thePlayer.addChatMessage(new TextComponentString("ToggleSprint " + (gameSettings.getLuckyOptionB("toggleSprint") ? "Ativado" : "Desativado")));
        }
    ...
    private void middleClickMouse() {
        if (this.objectMouseOver != null && this.objectMouseOver.typeOfHit != RayTraceResult.Type.MISS) {
            ...
            if (itemstack.getItem() == null) {
                ...
                LOGGER.warn("Picking on: [{}] {} gave null item", this.objectMouseOver.typeOfHit, s1);
            } else {
        ...
        if (i != 0 && !Keyboard.isRepeatEvent()) {
            if (!(this.currentScreen instanceof GuiControls) || ((GuiControls) this.currentScreen).time <= getSystemTime() - 20L) {
                if (Keyboard.getEventKeyState()) {
                    keysCounter.put(Keyboard.getEventKey(), (keysCounter.get(Keyboard.getEventKey()) == null ? 0 : keysCounter.get(Keyboard.getEventKey())) + 1);
