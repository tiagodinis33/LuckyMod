...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float partialTicks)
    {
        ...
        if(!mc.gameSettings.showDebugInfo){
            renderMouseStrokes(10, 70, "LMB", true, 0);
            renderMouseStrokes(54, 70, "RMB", true, 1);
            renderKeyStrokes(38, 20, "W", true, Keyboard.KEY_W);
            renderKeyStrokes(10, 45, "A", false, Keyboard.KEY_A);
            renderKeyStrokes(38, 45, "S", false, Keyboard.KEY_S);
            renderKeyStrokes(66, 45, "D", false, Keyboard.KEY_D);
        }
    }
    private void renderKeyStrokes(int posX, int posY, String name, boolean renderCps, int keycode){
        drawGradientRect(posX, posY, 20+posX+getFontRenderer().getStringWidth(name), getFontRenderer().FONT_HEIGHT + 15 + posY,Keyboard.isKeyDown(keycode)?0xffffffff : -1072689136, Keyboard.isKeyDown(keycode)?0xffffffff : -804253680);
        drawCenteredString(getFontRenderer(), name, posX+((20+getFontRenderer().getStringWidth(name))/2.0f), renderCps? 5+posY :posY+(20/2.0f), Keyboard.isKeyDown(keycode) ?  0:0xffffff);
        if(renderCps)
        {
            GL11.glPushMatrix();
            GL11.glScalef(0.6f, 0.6f, 0.6f);
            getFontRenderer().drawString("CPS: " + mc.keyboardCPS.get(keycode), 10+(posX / 0.6f), 30+(posY/ 0.6f), Keyboard.isKeyDown(keycode) ?  0:0xffffff, !Keyboard.isKeyDown(keycode));
            GL11.glPopMatrix();
        }
    }
    private void renderMouseStrokes(int posX, int posY, String name, boolean renderCps, int mouseButton){
        drawGradientRect(posX, posY, 20+posX+getFontRenderer().getStringWidth(name), 29 + posY,Mouse.isButtonDown(mouseButton)?  0xffffffff:-1072689136, Mouse.isButtonDown(mouseButton)?  0xffffffff:-804253680);

        drawCenteredString(getFontRenderer(), name, posX+((20+getFontRenderer().getStringWidth(name))/2.0f), renderCps? 5+posY :posY+(20/2.0f), Mouse.isButtonDown(mouseButton)  ?  0:0xffffff);
        if(renderCps){
            GL11.glPushMatrix();
            GL11.glScalef(0.6f, 0.6f, 0.6f);
            drawCenteredString(getFontRenderer(), "CPS: " + (mouseButton == 0? mc.cpsLeft : mc.cpsRight), 30+(posX / 0.6f), 30+(posY/ 0.6f), Mouse.isButtonDown(mouseButton)  ?  0:0xffffff);
