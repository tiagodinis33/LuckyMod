...
public class GameSettings {
    ...
    public KeyBinding keyBindToggleFreeCam = new KeyBinding("key.freeCam", Keyboard.KEY_L, "key.categories.luckymod");
    ...
    public KeyBinding keyBindToggleSneak = new KeyBinding("key.toggleSneakKey", Keyboard.KEY_G, "key.categories.luckymod");
    public KeyBinding keyBindToggleSprint = new KeyBinding("key.toggleSprintKey", Keyboard.KEY_I, "key.categories.luckymod");
    ...
    private JsonObject luckyModGson;
     ...
     */
    public void setOptionValueLuck(String settingsOption, boolean value) {
        luckyModGson.addProperty(settingsOption, value);
    }

    public void setOptionValueLuck(String settingsOption, int value) {
        luckyModGson.addProperty(settingsOption, value);
    }

    public void setOptionValueLuck(String settingsOption, float value) {
        luckyModGson.addProperty(settingsOption, value);
    }

    public void setOptionValueLuck(String settingsOption, double value) {
        luckyModGson.addProperty(settingsOption, value);
    }

    public void setOptionValueLuck(String settingsOption, String value) {
        luckyModGson.addProperty(settingsOption, value);
    }

    public JsonObject getLuckyModGson() {
        return luckyModGson;
    }
    ...
    public String getKeyBinding(GameSettings.Options settingOption) {
        ...
        if (s != null) {
            ...
            if (settingOption.getEnumFloat()) {
                ...
                return settingOption == GameSettings.Options.SENSITIVITY ? (f == 0.0F ? s1 + I18n.format("options.sensitivity.min") : (f == 1.0F ? s1 + I18n.format("options.sensitivity.max") : s1 + (int) (f * 200.0F) + "%")) : (settingOption == GameSettings.Options.FOV ? (f1 == 70.0F ? s1 + I18n.format("options.fov.min") : (f1 == 110.0F ? s1 + I18n.format("options.fov.max") : s1 + (int) f1)) : (settingOption == GameSettings.Options.FRAMERATE_LIMIT ? (f1 == settingOption.valueMax ? s1 + I18n.format("options.framerateLimit.max") : s1 + (int) f1 + " fps") : (settingOption == GameSettings.Options.RENDER_CLOUDS ? (f1 == settingOption.valueMin ? s1 + I18n.format("options.cloudHeight.min") : s1 + ((int) f1 + 128)) : (settingOption == GameSettings.Options.GAMMA ? (f == 0.0F ? s1 + I18n.format("options.gamma.min") : (f == 1.0F ? s1 + I18n.format("options.gamma.max") : s1 + "+" + (int) (f * 100.0F) + "%")) : (settingOption == GameSettings.Options.SATURATION ? s1 + (int) (f * 400.0F) + "%" : (settingOption == GameSettings.Options.CHAT_OPACITY ? s1 + (int) (f * 90.0F + 10.0F) + "%" : (settingOption == GameSettings.Options.CHAT_HEIGHT_UNFOCUSED ? s1 + GuiNewChat.calculateChatboxHeight(f) + "px" : (settingOption == GameSettings.Options.CHAT_HEIGHT_FOCUSED ? s1 + GuiNewChat.calculateChatboxHeight(f) + "px" : (settingOption == GameSettings.Options.CHAT_WIDTH ? s1 + GuiNewChat.calculateChatboxWidth(f) + "px" : (settingOption == GameSettings.Options.RENDER_DISTANCE ? s1 + (int) f1 + " chunks" : (settingOption == GameSettings.Options.MIPMAP_LEVELS ? (f1 == 0.0F ? s1 + I18n.format("options.off") : s1 + (int) f1) : (f == 0.0F ? s1 + I18n.format("options.off") : s1 + (int) (f * 100.0F) + "%"))))))))))));
    ...
    public boolean getLuckyOptionB(String key) {
        if (!luckyModGson.has(key)) {
            luckyModGson.addProperty(key, false);
            return false;
        }
        return luckyModGson.get(key).getAsBoolean();
    }

    public String getLuckyOptionStr(String key) {
        if (!luckyModGson.has(key)) {
            luckyModGson.addProperty(key, "");
            return "";
        }
        return luckyModGson.get(key).getAsString();
    }

    public int getLuckyOptioni(String key) {
        if (!luckyModGson.has(key)) {
            luckyModGson.addProperty(key, 0);
            return 0;
        }
        return luckyModGson.get(key).getAsInt();
    }

    public float getLuckyOptionf(String key) {
        if (!luckyModGson.has(key)) {
            luckyModGson.addProperty(key, 0.0f);
            return 0.0f;
        }
        return luckyModGson.get(key).getAsFloat();
    }
    ...
    public void loadOptions() {
        File luckyJson = new File(mc.mcDataDir.getPath() + "/LuckyMod/config.json");
        FileReader luckyJsonReader;
        FileWriter luckyJsonWriter;
        try {
            luckyJson.getParentFile().mkdirs();
            if (luckyJson.createNewFile()) {
                luckyJsonWriter = new FileWriter(luckyJson);

                luckyJsonWriter.write("{\n"
                        + "		\"enablePanorama\":true,\n"
                        + " 	\"toggleSneak\":false,\n"
                        + "		\"toggleSprint\":false,\n"
                        + "		\"blackBG\":false"
                        + "}");
                luckyJsonWriter.close();
            }
            luckyJsonReader = new FileReader(luckyJson);
            JsonParser parser = new JsonParser();

            luckyModGson = parser.parse(luckyJsonReader).getAsJsonObject();

            luckyJsonReader.close();
        } catch (IOException e) {
            ...
            for (String s1 : nbttagcompound.getKeySet()) {
                ...
                try {
                    ...
                    if ("resourcePacks".equals(s1)) {
                        this.resourcePacks = GSON.fromJson(s2, TYPE_LIST_STRING);
                    ...
                    if ("incompatibleResourcePacks".equals(s1)) {
                        this.incompatibleResourcePacks = GSON.fromJson(s2, TYPE_LIST_STRING);
                    ...
                    for (KeyBinding keybinding : this.keyBindings) {
                        if (s1.equals("key_" + keybinding.getKeyDescription())) {
                            if (Reflector.KeyModifier_valueFromString.exists() && s2.indexOf(58) != -1) {
                                ...
                                Reflector.call(keybinding, Reflector.ForgeKeyBinding_setKeyModifierAndCode, object, Integer.valueOf(Integer.parseInt(astring[0])));
                            } else {
    ...
    public void saveLuckyModOptions() {
        try {
            FileWriter luckyJson = new FileWriter(mc.mcDataDir.getPath() + "/LuckyMod/config.json");
            luckyJson.write(GSON.toJson(luckyModGson));
            luckyJson.close();
        } catch (IOException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
